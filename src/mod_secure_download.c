...
SETDEFAULTS_FUNC(mod_secdownload_set_defaults) {
	...
	config_values_t cv[] = {
		...
		{ "secdownload.use-ip-addr",       NULL, T_CONFIG_BOOLEAN, T_CONFIG_SCOPE_CONNECTION },      /* 4 */
		{ "secdownload.use-time",          NULL, T_CONFIG_BOOLEAN, T_CONFIG_SCOPE_CONNECTION },      /* 5 */
	...
	for (i = 0; i < srv->config_context->used; i++) {
		...
		cv[4].destination = &(s->use_ip_addr);
		cv[5].destination = &(s->use_time);
	...
	for (i = 1; i < srv->config_context->used; i++) {
		...
		for (j = 0; j < dc->value->used; j++) {
			...
			if (buffer_is_equal_string(du->key, CONST_STR_LEN("secdownload.secret"))) {
				...
				PATCH(timeout);
			} else if (buffer_is_equal_string(du->key, CONST_STR_LEN("secdownload.use-ip-addr"))) {
				PATCH(use_ip_addr);
			} else if (buffer_is_equal_string(du->key, CONST_STR_LEN("secdownload.use-time"))) {
				PATCH(use_time);
			}
		}
	}
...
URIHANDLER_FUNC(mod_secdownload_uri_handler) {
	...
	if (p->conf.use_time) {
		...
		/* There was no ts_str; go straight to rel_uri */
		rel_uri = md5_str + 32;
	}
	...
	if (p->conf.use_ip_addr) {
		char * addrbuff;
		// Appends IP address: added by Peter Xu
		switch (con->dst_addr.plain.sa_family) {
			case AF_INET:
				addrbuff = malloc(INET_ADDRSTRLEN + 1);
				inet_ntop(AF_INET, &con->dst_addr.ipv4.sin_addr, addrbuff, INET_ADDRSTRLEN);
				break;
#ifdef HAVE_IPV6
			case AF_INET6:
				addrbuff = malloc(INET6_ADDRSTRLEN + 1);
				inet_ntop(AF_INET6, &con->dst_addr.ipv6.sin6_addr, addrbuff, INET6_ADDRSTRLEN);
				break;
#endif
		...
		buffer_append_string(p->md5, addrbuff);
		free(addrbuff);
	}
